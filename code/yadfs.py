...
def check_path_exists_in_yadfs(path):
...
def create_path_in_yadfs(destination_file_path):
    if not check_path_exists_in_yadfs(destination_file_path):
...
def create_file_in_yadfs(destination_file_path):
    if not check_path_exists_in_yadfs(destination_file_path):
        components = destination_file_path.split('/')
        components = list(filter(bool, components))
...
def mkdir(*vargs):
    path = vargs[0]
    status = create_path_in_yadfs(path)
    if status:
        update_namenode_filesystem_info()
        return True
    else:
...
def ls(*vargs):
    ...
    if len(vargs) > 1 and vargs[1].lower() == '-r':
        recursive = True
    status = check_path_exists_in_yadfs(path)
    if status:
        components = path.split('/')
        components = list(filter(bool, components))
        if components:
...
def rm(*vargs):
    path = vargs[0]
    if check_path_exists_in_yadfs(path):
        components = path.split('/')
        components = list(filter(bool, components))
        current = args.FILESYSTEM
        for component in components[:-1]:
        ...
        if components[-1] in current:
            if current[components[-1]] is None:
                ...
                update_namenode_filesystem_info()
                file_id = get_file_id_from_yadfs_file_path(path)
...
def get_file_id_from_yadfs_file_path(file_path):
    for file_id in args.FILE_INFO:
...
def upload(*vargs):
    ...
    if file_size is None:
        ...
        check_file_created = create_file_in_yadfs(destination_file_path)
            ...
            for i in range(1, num_blocks+1):
                ...
                for j in range(args.REPLICATION_FACTOR):
                    ...
                    if datanode_id is None:
                        ...
                        log_datanode(
                            datanode_id, destination_block_name, "upload")
...
def cat(*vargs):
    file_path = vargs[0]
    if check_path_exists_in_yadfs(file_path):
        file_id = get_file_id_from_yadfs_file_path(file_path)
        if file_id is None:
...
def run(*vargs):
    ...
    if not check_path_exists_in_yadfs(input_file_path):
        print(f"Error: {input_file_path} not found")
        return False
    if check_path_exists_in_yadfs(output_dir_path):
        print(f"Error: {output_dir_path} already present")
        return False

    file_id = get_file_id_from_yadfs_file_path(input_file_path)
    if file_id is None:
        print("Error: File not found.")
        return False
